import { BadRequestException, CallHandler, ConflictException, ExecutionContext, Logger, NestInterceptor } from "@nestjs/common";
...
export class IdempotencyInterceptor implements NestInterceptor {
    ...
    async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<unknown>> {
        ...
        const cacheState = await this.redisService.get<string | unknown>(redisKey);


        // STATE A : It's currently executing in another thread
        if (cacheState === 'PROCESSING') {
            this.logger.warn(`Idempotency collison detected for this key: ${idempotencyKey}`);
            throw new ConflictException('This request is currently being processed. Please wait.')
        }
        ...
        if (cacheState) {
            this.logger.log(`Idempotency cache hit. Returning cache for : ${idempotencyKey}`);
        ...
        return next.handle().pipe(
            tap(async (responsePayload: unknown) => {
                try {
                    // The transaction was deeply successful. Save the exact HTTP JSON payload
                    // back to Redis for 24 hours
                    await this.redisService.set(redisKey, responsePayload, { ttl: ms(this.idempotency_TTL) / 1000 })
                } catch (error) {
            ...
            catchError((error: unknown) => {
                // The business logic faile, so the transaction never occured
